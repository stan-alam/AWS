...
file="/home/ubuntu/Stan/ami.txt"     #the file where you keep your string name
...
$DIALOG --backtitle "Select the number of Bees" \
	--title "Bees Selecter" --clear \
        --radiolist "Choose the number of Bees you wish to bring up " 20 61 5 \
        "4"    "Four Bees" off \
        "5"    "Five Bees" off \
        "6"    "Six Bees" off \
        "7"    "Seven Bees" off \
...
case $retval in
  0)
       ...
       ~/beeswithmachineguns/bees up -s 4 -g public -i {$ami} -t m3.medium -k DW_KeyPair -l ubuntu;

       fi

       if [ $choice = "5" ]; then
             echo Bringing up 5 bees
        ~/beeswithmachineguns/bees up -s 5 -g public -i ami-06b7886e -t t2.micro -k DW_KeyPair -l ubuntu;
       fi

       if [ $choice = "6" ]; then
             echo Bringing up 6 Bees
        ~/beeswithmachineguns/bees up -s 6 -g public -i ami-06b7886e -t t2.micro -k DW_KeyPair -l ubuntu;
       fi

       if [ $choice = "7" ]; then
             echo Bringing up 7 Bees
        ~/beeswithmachineguns/bees up -s 7 -g public -i ami-06b7886e -t t2.micro -k DW_KeyPair -l ubuntu;
       fi
